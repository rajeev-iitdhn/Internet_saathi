def automation_biases():
  ...
  for pro in form_pro.index:
    ...
    for m in forms:
      ...
      md=biased_questions(df)
      md["fid"]=md.insert(0,"formId",m)
      #print(md.sort_values("BiasParameter",ascending=False).T.to_dict().values())
      pingu.extend(md.sort_values("BiasParameter",ascending=False).T.to_dict().values())
